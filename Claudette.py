...
from .chat.ask_question import (
    ClaudetteAskNewQuestionCommand,
    ClaudetteAskQuestionCommand,
)
from .chat.chat_history import (
    ClaudetteClearChatHistoryCommand,
    ClaudetteExportChatHistoryCommand,
...
from .settings.select_system_message_panel import (
    ClaudetteSelectSystemMessagePanelCommand,
)
...
class ClaudetteFocusListener(sublime_plugin.EventListener):
    ...
    def _update_current_chat_status(self, view):
        ...
        for other_view in window.views():
            if other_view.id() != view.id() and other_view.settings().get(
