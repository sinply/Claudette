...
def claudette_get_current_directory(window):
    """Return directory of the active view or last used directory."""
...
def claudette_validate_and_sanitize_message(message):
    ...
    cleaned_message = {"role": message["role"], "content": content}
    ...
    def load_history(self, path):
        ...
        try:
            ...
            if (
                ...
                or "messages" not in import_data
            ):
            ...
            valid_messages = [
                msg
                for msg in messages
                if claudette_validate_and_sanitize_message(msg)
            ]
            if not valid_messages:
                raise ValueError("No valid messages found in import file")

            ask_cmd = ClaudetteAskQuestionCommand(self.window)
            ...
            chat_view = ClaudetteChatView.get_instance(
                self.window, ask_cmd.settings
            )
            if not chat_view:
                return
            ...
            sublime_view.settings().set(
                "claudette_conversation_json", json.dumps(valid_messages)
            )
            ...
            sublime.status_message(
                f"{PLUGIN_NAME}: Chat history imported successfully"
            )
...
class ClaudetteExportChatHistoryCommand(sublime_plugin.WindowCommand):
    def run(self):
        try:
            ...
            self.conversation_json = view.settings().get(
    ...
    def save_history(self, path):
        ...
        try:
            ...
            export_data = {"messages": self.messages}

            with open(path, "w", encoding="utf-8") as f:
                json.dump(export_data, f, indent=2, ensure_ascii=False)

            sublime.status_message(
                f"{PLUGIN_NAME}: Chat history exported successfully"
            )
...
class ClaudetteClearChatHistoryCommand(sublime_plugin.WindowCommand):
    """WindowCommand so the menu stays enabled when no editor has focus."""
    ...
    def run(self):
        ...
        if current_chat_view:
            current_chat_view.settings().set(
                "claudette_conversation_json", "[]"
            )
